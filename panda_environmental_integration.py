...
def do_integration(frame, pose_kind, personSegModel, keypoints):
    ...
    if pose_kind == 1:
        ...
        if person_height / person_width <= _row * 2 / _col: #宽度优先
            ...
            frame = frame[0:_row, int((person_pos[2][1]+person_pos[3][1]-_col/2)/2):int((person_pos[2][1]+person_pos[3][1]+_col/2)/2)]
            mask = mask[0:_row, int((person_pos[2][1]+person_pos[3][1]-_col/2)/2):int((person_pos[2][1]+person_pos[3][1]+_col/2)/2)]
